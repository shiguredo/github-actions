...
- CUDA 13.x: `13.0.0`, `13.0.1`, `13.0.2`, `13.1.0`, `13.1.1`, `13.2.0`
...
rtCamp/action-slack-notify と引数名・メッセージレイアウトの互換性を保ちつつ、前回 failure から今回 success になった場合の Fixed 通知機能を追加しています。

Fixed 通知と別ジョブの失敗検出には `actions: read` 権限が必要です。

コミットメッセージの取得に GitHub API (`gh api`) を使用しているため、`actions/checkout` は不要です。通知専用の別ジョブとして実行することを推奨します。
...
<details>
<summary>基本的な通知（成功・失敗・Fixed を自動判定）</summary>
...
<details>
<summary>カスタムメッセージとチャネル指定</summary>
...
<details>
<summary>最小表示（Status フィールドのみ）</summary>
...
<details>
<summary>Fixed 通知（失敗 → 成功で自動検知）</summary>

Fixed 通知は前回のワークフロー実行結果を `gh run list` で取得するため、`actions: read` 権限と `github.token` が必要です。`github.token` はアクション内部で自動的に使用されるため、明示的に渡す必要はありません。
